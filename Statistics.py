...
class Statistics:
    ...
    def __init__(self, _lambda, mu, v, n, data):
        self.__lambda = _lambda
        self.__mu = mu
        self.__v = v
        self.__n = n
        self.__stat = np.array(data[0])
        self.__total_requests = np.array(data[1])
        self.__total_time = np.array(data[2])
    ...
    ##### E
    def get_e_prob(self):
        P = [len(self.__stat[self.__stat == index]) / len(self.__stat) for index in range(self.__n+1)]
        for index, p in enumerate(P):
            print('p{0}: {1}'.format(index, p))
        # self.show_chart(P)
        return len(self.__stat[self.__stat == 0]) / len(self.__stat), len(
            self.__stat[self.__stat == self.__n]) / len(self.__stat)

    def get_e(self):
        return [len(self.__stat[self.__stat == index]) / len(self.__stat) for index in range(self.__n +1)]
    ...
    ##### T    
    def get_t_prob(self):
        mu_tilda = 1/ (1/self.__mu + 1/self.__v )
        teta = self.__lambda / mu_tilda
        ...
        pot = pn
        return round(p0, 15), round(pot, 15), self.avg_total_t(p0, pn)
    ...
    def avg_total_t(self, p0, pn):
        ...
        return sum([index * p0 * (teta ** index) / math.factorial(index) for index in range(1, self.__n + 1)])
    ####
    ...
    def sustainability_test(self,n, m, lambd, mu, v,time):
        ...
        plt.bar(X-0.05, p, width=0.1, label = 'theoretical')
        # ax1.set_title('Theoretical_Histogram') 
        plt.bar(X+0.05, empirical_characteristic, width=0.1, label = 'empirical')
    ...
    def generate(self):
        ...
        print('Time of warming up : {}'.format(self.__v))
        ...
        mu_tilda = 1/ (1/self.__mu + 1/self.__v )
        avg_off_e = Q_e * self.__lambda / mu_tilda
        ...
        print("Theoretical average number of applications served in the QS : {}".format(t_prob[2]))
        mu_tilda = 1/ (1/self.__mu + 1/self.__v )
        avg_off_t = Q_t * self.__lambda / mu_tilda
        print("Theoretical average occupied channels : ", avg_off_t)
        avg_SMO_t = t_prob[2] / self.__lambda
